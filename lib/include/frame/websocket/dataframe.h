...
// 这个将被弃用，转用recvframe 和 sendframe
class DataFrame
{
	...
	static const unsigned int MAX_DATA_LEN = 65536;
...
public:
	...
	bool			FromRecvFrame(const char *msg);
	// 构造发送帧
	void			ToSendFrame(const char* src, unsigned int src_len, char* des, unsigned int &des_len, bool mask = false, bool init_data = false);

	const char		*GetContent(){ return m_data; }

	unsigned int	GetContentLen(){ return m_data_len; }
