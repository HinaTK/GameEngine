...
: net_id(_net_id)
{				
	static char extend_data[DataFrameHeader::MID_EXTEND_LEN] = { 0 };		// 一般没有数据
	static char mask_data[DataFrameHeader::MASK_LEN] = { 0 };			// 一般没有数据
	...
	unsigned int frame_length = DataFrameHeader::HEADER_LEN + extend_len + mask_len + src_len;
	...
	if (extend_len > 0)
	{
		memcpy(data_frame.mem + DataFrameHeader::HEADER_LEN, extend_data, extend_len);
	}
	if (mask_len > 0)
	{
		memcpy(data_frame.mem + DataFrameHeader::HEADER_LEN + extend_len, mask_data, DataFrameHeader::MASK_LEN);
	}
}
