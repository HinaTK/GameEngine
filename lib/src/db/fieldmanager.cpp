...
bool FieldManager::Save(char *name)
{
	int field_size = m_manager.size()
	if (field_size <= 0)
	{
		return true;
	}
	...
	// 并传入角色id
	MysqlPrepareDynamic mp(33333, field_size);
	std::string sql = "REPLACE INTO " + name + "(";
	...
	for (int i = 0; i < field_size; ++i)
	{
		...
		data += "?,";
		mp.BindText(num, temp[num].c_str(), temp[num].size());
		++num;
	}
