...
public:
	...
	public:
		...
		V* operator->() { return &node->val; }
		V& operator*() { return node->val; }

		bool operator==(const Iterator<V, Handle> &right) const { return ( index == right.index && m_handle == right.m_handle); }
		bool operator!=(const Iterator<V, Handle> &right) const 
		{ 
			return ( index != right.index || m_handle != right.m_handle); 
		}
	...
	iterator	Begin()
	{ 
		return iterator(0, m_hash_list[m_begin_index], this); 
	}
...
bool game::Hash<K, V>::Push(K key, V &val)
{
	...
	{
		m_hash_list[real_key] = (ListNode *)m_memory_pool.Alloc();
		m_hash_list[real_key]->key = key;
		m_hash_list[real_key]->val = val;
	...
	real_key < m_begin_index ? m_begin_index = real_key : 0;
	return true;
}
...
{
	unsigned int real_key = key % m_size;
	ListNode *node = m_hash_list[real_key];
	...
	while (node != NULL)
	{
		if (node->key == key)
		{
			...
			else
			{
				...
				if (m_begin_index == real_key)
				{
					for (unsigned int i = m_begin_index + 1; i < m_size; ++i)
					{
