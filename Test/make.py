...
INCPATH 	= INCROOTPATH + " " + INCROOTPATH + "/lib/include";
SYSLIBS		= " -lmysqlclient -lpthread"
LIBS		= LIBROOTPATH + "/lib/linux " + "-lFrame -lLog -lTimeManager -lScript -lCommon -lTinyxml" + SYSLIBS;
CXX			= "g++";
FLAGS		= '-g -Wall';
...
for val in OBJ2SRC:
	f.write(val[0] + ":" + val[1] +"\n");
	f.write("	rm -f $@\n");
	f.write("	$(CXX) -fpic -g -c $(INCPATH) $< -o $@\n");
	f.write("\n");
